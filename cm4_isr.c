...
//uint32_t system_time_counter = 0; // 系统时间计数器（单位：ms）
// **************************** PIT中断函数 ****************************
 ...
 //  display_control();
   // show_flag1=0;
    ...
    // ips114_draw_line (0, ImageParameter.OFFLine, 158,ImageParameter.OFFLine, RGB565_RED);
      //display_control();  // 自动处理显示和清屏  20ms进一次
   // display_control();
    anjian_pid_change();
    Control();
  //  display_control();
    wireless_data_handler();
            //速度环pid图像
    //wireless_printf("%d,%d,%d,%d\n",Motor_Left_pid.present_value[0],Motor_Right_pid.present_value[0],Motor_Left_pid.set_value[0],Motor_Right_pid.set_value[0]);
   //
    ...
    // ips114_show_gray_image(0,0,(const uint8 *)Compressed_image,80,60,80*2,60*2,ImageParameter.Threshold);      
     ...
     // ips114_show_int(162, 31, show_count, 6);
    
     
 //   p = 666; //++;      // 时间计数器+1
 /*   p++;
    if(p<500) {Motor_Left_pid.set_value[0]=50;Motor_Right_pid.set_value[0]=50;}
    if(p>500&&p<1000) Motor_Left_pid.set_value[0]=0,Motor_Right_pid.set_value[0]=0;
    if(p>=1000)  p = 0 ;*/
